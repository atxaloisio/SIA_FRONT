 ...
 */
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente excluir o acondicionamento: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._acondicionamentoService.deleteAcondicionamento(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'Acondicionamento excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
