...
import { DsContratante } from './dscontratante';
...
@Component({
  ...
  styleUrls: ['./contratante-list.component.css']
})
export class ContratanteListComponent implements OnInit, AfterViewInit {
  displayedColumns = ['id', 'descricao'];
  // displayedColumns = ['id', 'codigo', 'descricao'];
  dataSource: DsContratante | null;
  selectedRowIndex = -1;
  ...
  obterContratantes() {
    ...
    // this._contratanteService.getContratantes(token).subscribe(data => {
    //   this.contratantes = data.data;
    //   console.log(data);
    //   console.log(this.contratantes.length);
    //   console.log(token);
    // });
  }
  ...
  dblck(row) {
    this._router.navigate(['/contratantes/contratante', {id: row.id}]);
  }
 ...
 */
  ...
  incluirRegistro() {
    this._router.navigate(['/contratantes/contratante']);
  }


  editarRegistro() {
    if (this.validaSelecao()) {
      this._router.navigate(['/contratantes/contratante', {id: this.selectedRow.id}]);
      this.ngOnInit();
      this.selectedRowIndex = -1;
      this.selectedRow = null;
    }
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente excluir o contratante: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._contratanteService.deleteContratante(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'Contratante excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
  ...
  ngOnInit() {
    ...
    this.dataSource = new DsContratante(this._tokenManager, this._contratanteService, this.paginator, this.sort);
