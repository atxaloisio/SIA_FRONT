 ...
 */
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente excluir o contratocliente: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._contratoclienteService.deleteContratoCliente(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'Contrato de Cliente excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
