 ...
 */
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente excluir o contratofornecedor: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._contratofornecedorService.deleteContratoFornecedor(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'Contrato de Fornecedor excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
