 ...
 */
  ...
  aprovarRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente Aprovar a ordem pagamento: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._ordempagamentoService.aprovarOrdemPagamento(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'OrdemPagamento Aprovada com sucesso.');
                this.ngOnInit();
              },
              error => {
