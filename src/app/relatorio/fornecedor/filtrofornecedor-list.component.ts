...
import { DsFiltroFornecedor } from './dsfiltrofornecedor';
...
@Component({
  selector: 'app-filtrofornecedor',
  templateUrl: './filtrofornecedor-list.component.html',
  styleUrls: ['./filtrofornecedor-list.component.css']
})
export class FiltroFornecedorListComponent implements OnInit, AfterViewInit {
  displayedColumns = ['id',
                      'cnpj_cpf',
                      'razao_social',
                      'contato',
  ...
  dataSource: DsFiltroFornecedor | null;
  selectedRowIndex = -1;
 ...
 */
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente excluir o fornecedor: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._fornecedorService.deleteFornecedor(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'Fornecedor excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
  ...
  filtrarRegistro() {
    if (this.dataSource.data.length > 0) {
      this._relatorioService.Fornecedores = this.dataSource.data;
      this._router.navigate(['/relatorios/relatoriofornecedor']);
    }
  }
  ...
  ngOnInit() {
    ...
    this.dataSource = new DsFiltroFornecedor(this._tokenManager, this._fornecedorService, this.paginator, this.sort);
