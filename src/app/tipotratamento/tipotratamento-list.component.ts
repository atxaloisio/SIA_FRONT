 ...
 */
  ...
  excluirRegistro() {
    if (this.validaSelecao()) {
      this.dialog.question('SIA', 'Deseja realmente excluir o tipotratamento: ' + this.selectedRow.id + '?').subscribe(
        result => {
          if (result.retorno) {
            this._tipotratamentoService.deleteTipoTratamento(this._tokenManager.retrieve(), this.selectedRow.id).subscribe(
              data => {
                this.dialog.success('SIA', 'TipoTratamento excluído com sucesso.');
                this.ngOnInit();
              },
              error => {
